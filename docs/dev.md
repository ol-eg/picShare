...
`alembic/` is bind-mounted into the `app` container, so autogenerated migration
files land on disk automatically and are version-controlled. Always commit the
generated file along with the model change.

## First-time database setup

A freshly created database has no tables until the migrations are applied. The
`alembic/versions/` directory ships with an initial migration that creates the
`users`/`images` tables:
...
so they check the live code on disk). `alembic/` is bind-mounted too, so
migrations are always generated on disk and version-controlled.
